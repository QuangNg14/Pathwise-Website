...
export async function GET(request, { params }) {
  try {
    ...
    const post = {
      id: docSnap.id,
      ...data,
      views: incrementView ? (data.views || 0) + 1 : (data.views || 0),
...
export async function PUT(request, { params }) {
  try {
    ...
    if (image && image.size > 0) {
      ...
      const oldPublicId = docSnap.data().cloudinaryPublicId;
      if (oldPublicId) {
        try {
...
export async function DELETE(request, { params }) {
  try {
    ...
    const cloudinaryPublicId = docSnap.data().cloudinaryPublicId;
